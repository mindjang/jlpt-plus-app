...
### 🔒 보안 강화
- Firestore Rules 전체 재작성: billing 컬렉션 클라이언트 접근 완전 차단
- billingKey 등 민감 정보 서버 전용 처리
- 컬렉션별 세분화된 접근 권한 설정 완료

### 💰 결제/멤버십 개선
- PaywallOverlay → My 페이지 결제 모달로 통합 (테스트 모드 라벨 추가)
- 무료 세션 소진 로직 강화 (시작 시 예약, 중단 방지)
- "준비 중" 메시지 최소화 및 actionable CTA로 교체

### 🎯 사용자 경험 개선
- 게스트 맛보기 세션 추가 (로그인 없이 N5 단어 5개 체험)
- 홈 화면 CTA 계층 명확화 (게스트/로그인 사용자 분리)
- 통계 페이지 "어휘력" 탭 숨김 처리 (미구현 기능)

### ⚡ 성능 최적화
- 단어/한자 데이터 지연 로딩 구현
- 레벨/탭별 동적 import로 초기 번들 크기 대폭 감소
- 데이터 캐싱으로 재방문 시 빠른 로딩
