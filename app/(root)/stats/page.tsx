...
function StatsContent() {
  ...
  return (
    ...
    >
      <div className="w-full overflow-hidden bg-page min-h-screen relative">
        <AppBar title="학습 통계" onBack={() => router.back()} />

        {/* 배경 장식 그래픽 */}
        <div className="absolute bottom-0 right-0 w-64 h-64 pointer-events-none opacity-[0.03] select-none translate-x-12 translate-y-12">
          <img
        ...
        <div className="flex flex-col gap-4 p-4 pb-20 relative z-10">
          {/* 메인 탭 */}
          <div className="flex gap-2 bg-surface rounded-lg border border-divider p-1">
            <button
              onClick={() => setActiveTab('period')}
              className={`flex-1 py-2 rounded-lg text-body font-medium transition-colors ${activeTab === 'period'
            ...
            <button
              ...
              className={`flex-1 py-2 rounded-lg text-body font-medium transition-colors ${activeTab === 'category'
                  ? 'bg-primary text-white shadow-sm'
                  : 'text-text-sub active:bg-gray-50'
                }`}
            >
