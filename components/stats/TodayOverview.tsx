...
function StatCard({ icon, label, value, iconBg = 'bg-gray-100' }: StatCardProps) {
  return (
    <motion.div
      ...
      className="bg-surface rounded-lg shadow-soft p-3"
    >
...
export function TodayOverview() {
    ...
    return (
      <div className="space-y-4">
        <div className="grid grid-cols-2 gap-3">
          {[1, 2, 3, 4].map((i) => (
            <div key={i} className="bg-surface rounded-lg shadow-soft p-4 h-24 animate-pulse" />
          ))}
        </div>
      </div>
    )
  ...
  return (
    <div className="space-y-3 relative overflow-hidden">
      {/* 배경 장식 그래픽 */}
      <div className="absolute -bottom-4 -right-4 w-32 h-32 pointer-events-none opacity-[0.04] select-none">
        <img
