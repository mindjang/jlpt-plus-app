...
function loadExistingData() {
  const filePath = path.join(outputDir, 'n2_naver.ts');
  ...
  try {
    ...
    const match = content.match(/export const n2NaverWords: NaverN2Word\[\] = \[([\s\S]*?)\]/);
...
function generateTypeScriptFile() {
  ...
  let content = `// 네이버 일본어 사전 N2 단어 데이터 (${data.length}개)\n`;
  content += `// API: https://ja.dict.naver.com/api/jako/getJLPTList?level=2\n`;
  content += `// 품사별 수집: ${PARTS_CONFIG.map(c => `${c.part}(${c.pages}페이지)`).join(', ')}\n\n`;
  content += `export interface NaverN2Word {\n`;
  ...
  content += `}\n\n`;
  content += `export const n2NaverWords: NaverN2Word[] = [\n`;
