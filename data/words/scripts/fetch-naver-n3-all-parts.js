...
function loadExistingData() {
  const filePath = path.join(outputDir, 'n3_naver.ts');
  ...
  try {
    ...
    const match = content.match(/export const n3NaverWords: NaverN3Word\[\] = \[([\s\S]*?)\]/);
...
function generateTypeScriptFile() {
  ...
  let content = `// 네이버 일본어 사전 N3 단어 데이터 (${data.length}개)\n`;
  content += `// API: https://ja.dict.naver.com/api/jako/getJLPTList?level=3\n`;
  content += `// 품사별 수집: ${PARTS_CONFIG.map(c => `${c.part}(${c.pages}페이지)`).join(', ')}\n\n`;
  content += `export interface NaverN3Word {\n`;
  ...
  content += `}\n\n`;
  content += `export const n3NaverWords: NaverN3Word[] = [\n`;
