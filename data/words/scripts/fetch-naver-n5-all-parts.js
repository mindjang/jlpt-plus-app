...
function loadExistingData() {
  const filePath = path.join(outputDir, 'n5_naver.ts');
  ...
  try {
    ...
    const match = content.match(/export const n5NaverWords: NaverN5Word\[\] = \[([\s\S]*?)\]/);
...
function generateTypeScriptFile() {
  ...
  let content = `// 네이버 일본어 사전 N4 단어 데이터 (${data.length}개)\n`;
  content += `// API: https://ja.dict.naver.com/api/jako/getJLPTList?level=4\n`;
  content += `// 품사별 수집: ${PARTS_CONFIG.map(c => `${c.part}(${c.pages}페이지)`).join(', ')}\n\n`;
  content += `export interface NaverN4Word {\n`;
  ...
  content += `}\n\n`;
  content += `export const n4NaverWords: NaverN4Word[] = [\n`;
...
async function main() {
  try {
    ...
    // 파일 저장
    const filePath = path.join(outputDir, 'n4_naver.ts');
