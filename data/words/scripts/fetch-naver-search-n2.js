...
const n2Path = path.join(__dirname, '../data/words/n2.ts');
const outputDir = path.join(__dirname, '../data/words/search-results');
const detailsDir = path.join(__dirname, '../data/words/details/n2');
...
function extractEntries() {
  try {
    const content = fs.readFileSync(n2Path, 'utf-8');
    const entries = [];
    ...
    console.error('n2.ts 파일 읽기 실패:', error.message);
    process.exit(1);
  }
}
...
async function main() {
  ...
  for (let i = 0; i < results.length; i += chunkSize) {
    ...
    const chunkFileName = `n2-details-chunk-${chunkIndex}.json`;
  ...
  const indexPath = path.join(detailsDir, 'n2-details-index.json');
  fs.writeFileSync(indexPath, JSON.stringify(indexData, null, 2), 'utf-8');
  console.log(`인덱스 저장: ${indexPath}`);

  // 백업 전체 JSON
  const allResultsPath = path.join(outputDir, `n2-search-results-${timestamp}.json`);
  fs.writeFileSync(allResultsPath, JSON.stringify(results, null, 2), 'utf-8');
  console.log(`백업 JSON 저장: ${allResultsPath}`);

  // 에러 로그
  if (errors.length > 0) {
    const errorsPath = path.join(outputDir, `n2-errors-${timestamp}.json`);
  ...
  const summaryPath = path.join(outputDir, `n2-summary-${timestamp}.json`);
