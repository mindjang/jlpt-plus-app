...
function extractEntries() {
  try {
    const content = fs.readFileSync(n3Path, 'utf-8');
    const entries = [];
    ...
    console.error('n3.ts 파일 읽기 실패:', error.message);
    process.exit(1);
  }
}
...
async function main() {
  ...
  for (let i = 0; i < results.length; i += chunkSize) {
    ...
    const chunkFileName = `n3-details-chunk-${chunkIndex}.json`;
  ...
  const indexPath = path.join(detailsDir, 'n3-details-index.json');
  fs.writeFileSync(indexPath, JSON.stringify(indexData, null, 2), 'utf-8');
  console.log(`인덱스 저장: ${indexPath}`);

  // 백업 전체 JSON
  const allResultsPath = path.join(outputDir, `n3-search-results-${timestamp}.json`);
  fs.writeFileSync(allResultsPath, JSON.stringify(results, null, 2), 'utf-8');
  console.log(`백업 JSON 저장: ${allResultsPath}`);

  // 에러 로그
  if (errors.length > 0) {
    const errorsPath = path.join(outputDir, `n3-errors-${timestamp}.json`);
  ...
  const summaryPath = path.join(outputDir, `n3-summary-${timestamp}.json`);
