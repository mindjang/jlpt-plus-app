...
/**
 * KanjiAliveEntry의 ID 생성 (SRS 시스템용)
 * 레벨과 인덱스로 일관된 ID 생성
 */
export function getKanjiId(entry: KanjiAliveEntry, level: string, index: number): string {
  return `${level}_K_${String(index + 1).padStart(4, '0')}`
}
