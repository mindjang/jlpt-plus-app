...
import { dayNumberToMinutes, daysToMinutes } from '../utils/dateUtils'
...
/**
 * lastReviewed 값을 분 단위로 변환 (레거시 형식 처리)
 * @param lastReviewed lastReviewed 값
 * @returns 분 단위 lastReviewed 값
 */
export function normalizeLastReviewed(lastReviewed: number): number {
  if (isLegacyFormat(lastReviewed)) {
    return dayNumberToMinutes(lastReviewed)
  }
  return lastReviewed
}
